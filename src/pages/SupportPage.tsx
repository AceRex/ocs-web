...
import { MessageSquare, Mail, Tag, FileText, CheckCircle2, AlertCircle, Upload } from "lucide-react"
          ...
          <AnimatePresence mode="wait">
            {!submitted ? (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="glass-card rounded-2xl p-8 shadow-xl shadow-purple-100/40"
              >
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="s-email" className="flex items-center gap-1.5 text-slate-700">
                  ...
                  {/* Subject */}
                  <div className="space-y-1.5">
                    <Label htmlFor="s-subject" className="flex items-center gap-1.5 text-slate-700">
                  ...
                  <div className="space-y-2.5">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {priorities.map((p) => (
                        ...
                        >
                          <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded-md", p.color)}>
                            {p.value.toUpperCase()}
                          </span>
                  ...
                  {/* Message */}
                  <div className="space-y-1.5">
                    <Label htmlFor="s-message" className="text-slate-700">
                      Message
                    </Label>
                    <Textarea
                      id="s-message"
                      ...
                      className="bg-white border-slate-200 min-h-[140px] focus-visible:ring-purple-400"
                      required
                    />
                  </div>
                  ...
                  {/* Attachment (UI only) */}
                  <div className="space-y-1.5">
                    <Label className="text-slate-700">Attachment <span className="text-slate-400 font-normal">(optional)</span></Label>
                    <label
                      htmlFor="s-file"
                      className="flex items-center justify-center gap-3 h-20 rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 cursor-pointer hover:border-purple-400 hover:bg-purple-50/30 transition-colors"
                    >
                      <Upload className="size-4 text-slate-400" />
                      <span className="text-sm text-slate-500">Click to attach a screenshot or log file</span>
                  ...
                  >
                    {loading ? (
                      ...
                      <>Submit Request <MessageSquare className="size-4" /></>
                    )}
                  </Button>
                </form>
              </motion.div>
              ...
              <motion.div
                ...
                className="glass-card rounded-2xl p-12 text-center shadow-xl shadow-purple-100/40 space-y-6"
              >
