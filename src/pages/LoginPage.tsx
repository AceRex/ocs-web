...
export default function LoginPage() {
  ...
  const promptGoogleEmailLogin = async () => {
    const inputEmail = window.prompt("Sign in with Google - Enter your Google Account Email:", email || "")
    ...
    try {
      ...
      if (isDesktopFlow && redirectUri) {
        const orgName = (data.user as any)?.churchName || (data.user as any)?.name || "Sanctuary"
        const tier = (data.user as any)?.subscriptionTier || "trial"
        const daysLeft = (data.user as any)?.trialRemainingDays ?? 60
        const callbackUrl = `${redirectUri}?token=${encodeURIComponent(data.token)}&state=${encodeURIComponent(state || "session_init")}&email=${encodeURIComponent(inputEmail.trim())}&org=${encodeURIComponent(orgName)}&tier=${encodeURIComponent(tier)}&days_left=${daysLeft}`
          ...
          >
            ...
            <motion.div variants={fadeUp} initial="hidden" animate="show" custom={0}>
              ...
              >
                ...
                {googleAuthMutation.isPending ? "Connecting with Google..." : "Continue with Google"}
              </Button>
            </motion.div>
